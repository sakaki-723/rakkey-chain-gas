// ============================================================
//【注意】
//・商品が13点以上の作家がいる場合、納品書は自動で商品行を追加してから記入します（開発者メタデータに前回の追加行数を記録し、切り替えのたびに一度元の12行に戻してから必要な行数だけ再追加する仕組みです）。合計金額などの計算式が正しい範囲を参照しているかは、導入後に一度必ず目視確認してください。
//・「商品一覧」シートは更新のたびに**全件書き直し**されます。手動で編集した内容は上書きされます。
//・納品書のH3以外のレイアウト（会社情報・件名・計算式など）はそのまま維持されます。
// ============================================================
...
const ARTIST_MASTER_SHEET = '作家マスタ';                   // 作家マスタシート
const PERIOD_ARTIST_SHEET = '会期ごと作家情報';              // 会期管理シート
...
function checkArtistsAgainstMaster(sheet, rows) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const masterSheet = ss.getSheetByName(ARTIST_MASTER_SHEET);
...
function fillInvoice(artistName) {
  ...
  // ---- 商品行が元のフォーマット（12行）に収まらない場合、行を自動追加する ----
  // BASE_ROWS：元フォーマットの商品行数。PRODUCT_START_ROW：商品行の開始行。
  // NOTE_ROW_BASE：備考欄の元の行番号（行追加分だけ下にずれる）。
  const BASE_ROWS = 12;
  const PRODUCT_START_ROW = 17;
  const LAST_TEMPLATE_ROW = PRODUCT_START_ROW + BASE_ROWS - 1; // 28行目（元フォーマットの最終商品行）
  const NOTE_ROW_BASE = 33;
  const META_KEY = 'invoiceExtraRows'; // 前回このシートに追加した行数を覚えておくための開発者メタデータのキー

  function getExtraRows(sheet) {
    const found = sheet.createDeveloperMetadataFinder().withKey(META_KEY).find();
    if (found.length === 0) return 0;
    return parseInt(found[0].getValue(), 10) || 0;
  }
  function setExtraRows(sheet, value) {
    sheet.createDeveloperMetadataFinder().withKey(META_KEY).find().forEach(function(md) { md.remove(); });
    sheet.addDeveloperMetadata(META_KEY, String(value));
  }

  // 前回追加した行が残っていれば削除し、いったん元の12行フォーマットに戻す
  // （挿入時と対称の位置で削除することで、下の合計計算式の参照範囲も自動で元に戻る）
  const prevExtraRows = getExtraRows(invoiceSheet);
  if (prevExtraRows > 0) {
    invoiceSheet.deleteRows(LAST_TEMPLATE_ROW, prevExtraRows);
  }

  const extraRows = Math.max(0, products.length - BASE_ROWS);
  if (extraRows > 0) {
    // 最終商品行（28行目）の直前に行を挿入する＝「28行目の上に挿入」扱いになるので、
    // 28行目を含む範囲を参照している数式（合計欄など）は自動で範囲が広がる
    invoiceSheet.insertRowsBefore(LAST_TEMPLATE_ROW, extraRows);

    // 挿入した行に、テンプレート商品行（挿入によって下にずれた元28行目）の書式・データ検証をコピーする
    const numCols = invoiceSheet.getMaxColumns();
    const templateRow = LAST_TEMPLATE_ROW + extraRows; // 元28行目が挿入分だけ下にずれた位置
    for (let r = 0; r < extraRows; r++) {
      invoiceSheet.getRange(templateRow, 1, 1, numCols)
        .copyTo(invoiceSheet.getRange(LAST_TEMPLATE_ROW + r, 1, 1, numCols));
    }
  }
  setExtraRows(invoiceSheet, extraRows);

  const totalRows = BASE_ROWS + extraRows; // 今回使う商品行の総数
  const noteRow = NOTE_ROW_BASE + extraRows; // 行追加分だけ下にずれた備考欄の行

  // 作家情報を書き込み
  invoiceSheet.getRange('H6').setValue(realName || artistName); // 本名
  ...
  invoiceSheet.getRange(PRODUCT_START_ROW, 6, totalRows, 1).clearDataValidations();

  // 商品行をクリア
  const emptyCol = Array.from({length: totalRows}, function() { return ['']; });
  invoiceSheet.getRange(PRODUCT_START_ROW, 2, totalRows, 1).setValues(emptyCol);
  invoiceSheet.getRange(PRODUCT_START_ROW, 5, totalRows, 1).setValues(emptyCol);
  invoiceSheet.getRange(PRODUCT_START_ROW, 6, totalRows, 1).setValues(emptyCol);
  invoiceSheet.getRange(PRODUCT_START_ROW, 7, totalRows, 1).setValues(emptyCol);
  invoiceSheet.getRange(PRODUCT_START_ROW, 8, totalRows, 1).setValues(emptyCol);
  ...
  invoiceSheet.getRange(PRODUCT_START_ROW, 2, products.length, 1).setValues(colB);
  invoiceSheet.getRange(PRODUCT_START_ROW, 5, products.length, 1).setValues(colE);
  invoiceSheet.getRange(PRODUCT_START_ROW, 6, products.length, 1).setValues(colF);
  invoiceSheet.getRange(PRODUCT_START_ROW, 7, products.length, 1).setValues(colG);
  invoiceSheet.getRange(PRODUCT_START_ROW, 8, products.length, 1).setValues(colH);
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('🔍 作家名で検索してNo.を入力', 'searchArtistAndFillNumber')
    .addToUi();
}
...
function checkArtistDuplicate(e) {
  const sheet = e.range.getSheet();
  if (sheet.getName() !== '作家マスタ') return;
  ...
  if (lastRow < 2) return;
  const allNames = sheet.getRange(2, 2, lastRow - 1, 1).getValues(); // B列全体（ヘッダー除く）
  ...
  for (let r = 0; r < numRows; r++) {
    ...
    // e.valueには頼らず実際のセル値を読み直す
    // （e.valueは単一セル編集時しかセットされず、複数セルへのコピペでは
    //   undefinedになってしまうため、貼り付けだと反応しない原因になっていた）
    const newValue = String(sheet.getRange(row, 2).getValue() || '').trim();
    if (!newValue) continue;

    const newTokens = extractTokens(newValue);
    const duplicates = [];
    for (let i = 0; i < allNames.length; i++) {
      const existing = String(allNames[i][0] || '');
      if (i + 2 === row) continue; // 自分自身はスキップ
      if (!existing || existing.trim() === '') continue;
      const existingTokens = extractTokens(existing);
      // 新規入力のいずれかのトークンが既存のいずれかのトークンと一致したら重複とみなす
      const isMatch = newTokens.some(function(nt) {
        return existingTokens.some(function(et) { return nt === et; });
      });
      if (isMatch) {
        duplicates.push(existing + '（' + (i + 2) + '行目）');
      }
    }

    if (duplicates.length > 0) {
      messages.push('入力値：' + newValue + '（' + row + '行目）\n一致する既存データ：\n' + duplicates.join('\n'));
    }
  }
...
function setupRoyaltyPeriodDropdown() {
  ...
  const invoiceSheet = ss.getSheetByName(INVOICE_SHEET_NAME);
  const periodSheet = ss.getSheetByName(PERIOD_ARTIST_SHEET);
  ...
  if (!periodSheet) {
    SpreadsheetApp.getUi().alert('シート「' + PERIOD_ARTIST_SHEET + '」が見つかりません。');
    return;
  }
...
// ============================================================
// メニュー実行用：選択中のセルに、作家名の一部から検索したNo.を入力する
// 会期管理シート（PERIOD_ARTIST_SHEET）のB列セルを選んだ状態で実行する想定
// 部分一致で候補を検索し、1件ならそのまま入力、複数件なら選択させる
// ============================================================
function searchArtistAndFillNumber() {
  const ui = SpreadsheetApp.getUi();
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const activeCell = ss.getActiveCell();
  const activeSheet = ss.getActiveSheet();

  if (activeSheet.getName() !== PERIOD_ARTIST_SHEET) {
    ui.alert('「' + PERIOD_ARTIST_SHEET + '」シートのNo.を入力したいセルを選んでから実行してください。');
    return;
  }

  const masterSheet = ss.getSheetByName(ARTIST_MASTER_SHEET);
  if (!masterSheet) {
    ui.alert('シート「' + ARTIST_MASTER_SHEET + '」が見つかりません。');
    return;
  }
  ...
  if (!keyword) {
    ui.alert('検索キーワードが空でした。');
    return;
  }

  // 作家マスタ：A列No. B列作家名（1行目はヘッダー）
  const masterData = masterSheet.getDataRange().getValues();
  const normalizedKeyword = normalizeToken(keyword);
  const matches = [];

  for (let i = 1; i < masterData.length; i++) {
    const masterArtist = String(masterData[i][1] || '').trim();
    if (!masterArtist) continue;
    if (normalizeToken(masterArtist).indexOf(normalizedKeyword) !== -1) {
      matches.push({ no: masterData[i][0], name: masterArtist });
    }
  }

  if (matches.length === 0) {
    ui.alert('「' + keyword + '」に一致する作家が見つかりませんでした。');
    return;
  }

  if (matches.length === 1) {
    activeCell.setValue(matches[0].no);
    ui.alert('No.' + matches[0].no + '（' + matches[0].name + '）を入力しました。');
    return;
  }

  // 複数件一致：候補を表示して作家番号で選んでもらう
  const list = matches.map(function(m) {
    return 'No.' + m.no + '　' + m.name;
  }).join('\n');

  const choiceResponse = ui.prompt(
    '⚠️ 複数の候補が見つかりました',
    list + '\n\n該当するNo.を入力してください：',
    ui.ButtonSet.OK_CANCEL
  );
  ...
  const inputNo = choiceResponse.getResponseText().trim();
  const selected = matches.find(function(m) { return String(m.no) === inputNo; });

  if (!selected) {
    ui.alert('入力されたNo.が候補の中に見つかりません。もう一度実行してください。');
    return;
  }

  activeCell.setValue(selected.no);
}
