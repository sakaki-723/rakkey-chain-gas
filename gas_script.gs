...
//・【第6回から】作家マスタ・会期ごと作家情報は別ファイル（マスタファイル）に分離した。
//  このファイルの下の方にある MASTER_FILE_ID に、マスタファイルのスプレッドシートIDを
//  必ず設定すること。未設定のままだと作家マスタ・会期ごと作家情報を参照する処理が全て失敗する。
//・【第6回から・重要】納品確認書H3（作家名プルダウン）の自動更新は、GASエディタの
//  トリガー画面から「onInvoiceArtistChange」をインストール型トリガー（編集時）として
//  手動設定しないと動かない。設定を忘れると、H3を変更しても何も起きず、エラーも出ない
//  （簡易トリガーは他ファイルへのアクセスが常に禁止されているため）。
// ============================================================
...
const MASTER_FILE_ID = 'ここにマスタファイルのスプレッドシートIDを入力してください';
